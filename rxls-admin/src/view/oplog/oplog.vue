...
import fromOplog from "@/components/system/from/from-oplog.vue";
import { useI18n } from "vue-i18n";
import RxTable from "@/components/system/rx_table/rx-table.vue";
import { CommonStateEnum } from "@/config/enum/common";
import { onMounted, ref } from "vue";
import { Oplog, PageQuery } from "@/api/system/oplog/type";
import { ClockCircleOutlined } from "@ant-design/icons-vue";
import { Role } from "@/api/system/role/type";
import { useTableHandler } from "@/components/system/rx_table/handler";
...
const { searchClick, dataLists, dataTotal, loading, delHandler, pageHandler } =
  useTableHandler<Oplog, PageQuery, null, null>("oplog", "id", formState);
...
const onSearch = (data?: PageQuery) => {
  if (data) {
    const size = formState.value.size;
