...
import SelectFromConfig from '@/components/business/from/select-from-config.vue'
import FromConfig from '@/components/business/from/from-config.vue'
import RxTable from '@/components/business/rx_table/rx-table.vue'
import { CommonStateEnum } from '@/config/enum/common'
import { onMounted, ref } from 'vue'
import { SysConfig, PageQuery, AddQuery, UpdateQuery } from '@/api/config/type'
...
import { ReqUpdateConfigState } from '@/api/config'
  ...
  ReqUpdateConfigState(id)
    .then((res) => {
      if (res.data.code == CommonStateEnum.OK) {
        ...
        if (index != -1) {
          dataLists.value[index].state = !dataLists.value[index].state
